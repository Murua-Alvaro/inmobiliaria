...
function RankList({rows,kind}){
  ...
  return <div className="ix-rank-list">{rows.map((r,i)=>{const value=kind==='inflation'?r.y:r.v;return <div className="ix-rank" key={r.n}><span className="ix-rank-num">{String(i+1).padStart(2,'0')}</span><div className="ix-rank-copy"><strong>{cleanName(r.n)}</strong><i><em style={{width:`${Math.max(2,Math.abs(value)/max*100)}%`}}/></i></div><b>{kind==='inflation'?signed(value,1):`${fmt(value,1)} pp`}</b></div>})}</div>
}
...
export default function Indicadores(){
  ...
  const filteredMaterials=useMemo(()=>{const q=materialQuery.trim().toLocaleLowerCase('es-MX');return q?allMaterials.filter(x=>x.n.toLocaleLowerCase('es-MX').includes(q)):allMaterials},[materialQuery])
  ...
  return <main className="ix-page">
    ...
    <section className="ix-card ix-pressure-card">
      ...
      <div className="ix-chart-layout"><TrendChart data={D.pressure} mode={chartMode}/><aside className="ix-chart-read"><span>LECTURA ACTUAL</span><h3>{pressureLabel}</h3><p>La inflación anual se ha re-acelerado desde el mínimo reciente de <b>{signed(recentLow.y,2)}</b> en {shortDate(recentLow.d)} y ya supera el ritmo observado hace doce meses.</p><div className="ix-read-grid"><div><span>Actual</span><strong>{signed(latest.y,2)}</strong></div><div><span>Hace 12m</span><strong>{signed(yearAgo.y,2)}</strong></div><div><span>Cambio mensual</span><strong>{signed(latest.m,2)}</strong></div><div><span>Volatilidad</span><strong>{fmt(latest.v,2)} pp</strong></div></div><div className="ix-implication"><span>Implicación</span><p>Conviene actualizar presupuestos y contingencias con mayor frecuencia cuando la inflación anual acelera, aun si la volatilidad mensual permanece contenida.</p></div></aside></div>
    </section>

    <section className="ix-card ix-material-card">
      <div className="ix-card-head"><div><span>02 · INSUMOS</span><h2>Explora materiales e insumos.</h2><p>Elige cualquiera de las 75 series del paquete y revisa su presión actual, volatilidad y riesgo de presupuesto.</p></div><label className="ix-select ix-select--material"><span>Material</span><select value={materialName} onChange={e=>setMaterialName(e.target.value)}>{allMaterials.map(x=><option key={x.n} value={x.n}>{cleanName(x.n)}</option>)}</select><b>⌄</b></label></div>
      ...
      <details className="ix-material-library">
        <summary><div><span>CATÁLOGO COMPLETO</span><strong>Abrir lista de materiales e insumos</strong><small>{allMaterials.length} series disponibles</small></div><b>+</b></summary>
        <div className="ix-library-body">
          <div className="ix-library-search"><input value={materialQuery} onChange={e=>setMaterialQuery(e.target.value)} placeholder="Buscar cemento, varilla, pintura, cable…"/><span>{filteredMaterials.length} resultados</span></div>
          <div className="ix-library-grid">{filteredMaterials.map(x=><button type="button" key={x.n} className={x.n===materialName?'active':''} onClick={()=>setMaterialName(x.n)}><span>{cleanName(x.n)}</span><div><strong>{signed(x.y,1)}</strong><small>12m</small></div></button>)}</div>
        </div>
      </details>
