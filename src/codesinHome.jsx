...
function CodesinMapHome() {
  ...
  const layers = useMemo(() => {
    ...
    const polygonLayer = new GeoJsonLayer({
      id: 'inmobiliaria-codesin-districts', data: geojson, pickable: true, stroked: true, filled: true, extruded: true, wireframe: true, opacity: 1,
      ...
      getLineColor: feature => String(feature?.properties?.district || '') === selected ? [0, 0, 0, 255] : [112, 112, 112, 210],
      lineWidthMinPixels: 1,
      material: { ambient: 0.88, diffuse: 0.34, shininess: 8, specularColor: [255, 255, 255] },
      updateTriggers: { getElevation: [selected, maxAgeb], getFillColor: [selected, maxAgeb], getLineColor: [selected] },
    })

    const labelLayer = new TextLayer({
      id: 'inmobiliaria-codesin-labels', data: districts, pickable: true,
      getPosition: district => [district.longitude, district.latitude], getText: district => cleanName(district.name), getSize: 11,
      sizeMinPixels: 9, sizeMaxPixels: 13,
      getColor: district => district.name === selected ? [255, 255, 255, 255] : [25, 25, 25, 235],
      getPixelOffset: [0, 18], fontWeight: 650, billboard: true, background: true,
      getBackgroundColor: district => district.name === selected ? [12, 12, 12, 230] : [255, 255, 255, 220],
      backgroundPadding: [5, 3], updateTriggers: { getColor: [selected], getBackgroundColor: [selected] },
    })
  ...
  return <main className="ch-stage">
    ...
    <div className="ch-map"><DeckGL initialViewState={INITIAL_VIEW} controller layers={layers}
      onClick={info => { const name = info?.object?.properties?.district ?? info?.object?.name ?? null; if (name) setSelected(String(name)) }}
      getTooltip={info => {
        ...
        return { html: `<div class="ch-tooltip"><span>DISTRITO CODESIN</span><strong>${cleanName(name)}</strong><small>${count ? `${count} AGEB integradas` : 'Seleccionar distrito'}</small></div>`, style: { background: 'transparent', padding: 0, border: 'none' } }
      }} /></div>
...
function Nav({page,go}) {
  return <nav className="ch-nav"><button type="button" className={page==='inicio'?'active':''} onClick={()=>go('inicio')}>Inicio</button><button type="button" className={page==='ageb'?'active':''} onClick={()=>go('ageb')}>AGEB / Demografía</button><button type="button" className={page==='indicadores'?'active':''} onClick={()=>go('indicadores')}>Indicadores</button></nav>
}
...
function App() {
  ...
  const badge = page==='ageb' ? 'PERFIL AGEB · 2020' : 'MAPA CODESIN · 13 DISTRITOS'

  if (page === 'indicadores') {
    return <div className="codesin-home codesin-home--indicators">
      <header className="ch-header ch-header--indicators-clean"><Nav page={page} go={go}/></header>
      <Indicadores/>
    </div>
  }
